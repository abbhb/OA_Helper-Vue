  ...
  const updateDeviceWay = (value, deviceId) => {
    ...
    if (value.length > 0) {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        needCliValue.push(value[i].trim())
      }
      for (
        let i = 0;
  ...
  <a-card class="add-bc">
                ...
                <div style="display: flex; flex-direction: column">
                  ...
                  <ul>
                    <li v-for="deviceId in selectDevice" :key="deviceId"
                      >{{ signinStore.getDeviceById(deviceId)?.name }}[方式:
                      <a-checkbox-group
                        :default-value="
    ...
    <div>
      <a-modal
        ...
        title="选择用户"
      >
        <add-user :pre-select-user="groupRule?.signinGroupRule?.rulesInfo?.userIds" @commit-user="userSelectSuccess" />
      </a-modal>
      <a-modal
