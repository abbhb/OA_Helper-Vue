...
const everyDayChange = (newC, OldC) => {
  ...
  groupRule.signinGroupRule.rulesInfo.kqsj = newList;
};
...
const shareChuli = (item, index) => {
  ...
  if (wukonggeS.endsWith(',')) {
    groupRule.signinGroupRule.rulesInfo.kqsj[index].xq = wukonggeS.slice(
      0,
      wukonggeS.length - 1
    );
  } else {
    groupRule.signinGroupRule.rulesInfo.kqsj[index].xq = wukonggeS;
  }

  // 去除其余选择该星期的
  for (let i = 0; i < groupRule.signinGroupRule.rulesInfo.kqsj.length; i++) {
    if (i === index) {
      // eslint-disable-next-line no-continue
      continue;
    }
    const asfaa = groupRule.signinGroupRule.rulesInfo.kqsj[i].xq.split(',');
    const newsax = [];
    if (asfaa.some((item) => wukongge.includes(item))) {
      ...
      groupRule.signinGroupRule.rulesInfo.kqsj[i].xq = newsax.join(',');
    }
  }
};
...
const addSigninGroup = async () => {
  if (groupRule.signinGroupRule.rulesInfo.signinWays.length === 0) {
    Message.info('请保证最少绑定一个设备且至少绑定一种方式!');
    return;
  }
  if (groupRule.signinGroup.name === '') {
    Message.info('请提供考勤组名称');
    return;
  }
  if (groupRule.signinGroupRule.rulesInfo.kqsj.length < 1) {
    Message.info('请至少选择一个时间考勤');
    return;
  }
  for (
    let j = 0;
    j < groupRule.signinGroupRule.rulesInfo.kqsj.length;
    j += 1
  ) {
    if (
      groupRule.signinGroupRule.rulesInfo.kqsj[j].xq === '' ||
      groupRule.signinGroupRule.rulesInfo.kqsj[j].bcId === ''
    ) {
      Message.info('请保证排版班次不能为空');
      return;
    }
  }
  const data = await addGroupRule(groupRule);
  Message.success(data.msg);
  emit('success');
...
const updateDeviceWay = (value, deviceId) => {
  ...
  if (value.length > 0) {
    for (
      let i = 0;
      i < groupRule.signinGroupRule.rulesInfo.signinWays.length;
      i += 1
    ) {
      if (
        groupRule.signinGroupRule.rulesInfo.signinWays[i].deviceId ===
        deviceId
      ) {
        zhaodao = 1;
        groupRule.signinGroupRule.rulesInfo.signinWays[i].type =
          value.join(',');
        break;
      }
    }
    if (zhaodao === 0) {
      // 没找到
      groupRule.signinGroupRule.rulesInfo.signinWays.push({
        deviceId: deviceId,
        type: value.join(','),
      });
    }
    ...
    for (
      ...
      if (
        ...
        copyDeviceList.push(
          groupRule.signinGroupRule.rulesInfo.signinWays[i]
        );
      }
    }
    if (zhaodao === 1) {
      groupRule.signinGroupRule.rulesInfo.signinWays = copyDeviceList;
    }
  }
};
</script>
