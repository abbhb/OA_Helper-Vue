<script setup lang="ts">
  ...
  import {
    downloadSigninCardData,
    get_signin_card_data,
    SigninUserCardDataResp,
    uploadSigninCardData,
  } from '@/api/signin';
  ...
  const getList = async () => {
    loading.value = true;
    const { data } = await get_signin_card_data(props.deviceId);
    list.value = data;
    loading.value = false;
  };

  const syncUpload = async () => {
    const data_datas: SigninUserCardDataResp[] = [];
    ...
    const rep = await uploadSigninCardData(data);
  ...
  const syncDownload = async () => {
    ...
    const rep = await downloadSigninCardData(data);
    Message.success(rep.msg);
  };
  const cleanData = () => {
    form.modelType = 'ADD';
    ...
    <div v-if="models === 'Upload'" class="model">
      <div>
        ...
        >
          ...
          <el-table-column
            ...
            width="200"
          />
          <el-table-column label="本地ID卡" align="center" prop="localExist">
          ...
          <el-table-column label="考勤机ID卡" align="center" prop="deviceExist">
            <template #default="scope">
      ...
      <div style="display: flex; flex-direction: column">
        ...
        <div
          ...
          "
        >
          <span style="font-size: 18px">本地ID卡</span>
