<script setup lang="ts">
  ...
  import {onBeforeUnmount, onMounted, reactive, ref, triggerRef} from 'vue';
  ...
  onMounted(async () => {
    ...
    } else if (appState.lastPrintDevice !== '' && printState.printDevice == null) {
  ...
  <div>
    ...
    >
      <div style="border: 1px #d7d6d6 solid; border-radius: 5px">
        ...
        <div
          v-else
          style="display: flex; flex-direction: row; padding: 3px 3px 3px 3px"
        >
    ...
    >
      <a-table :data="printState.printDeviceList">
        <template #columns>
          ...
          <a-table-column title="打印机描述" data-index="description"></a-table-column>
          <a-table-column title="状态">
            <template  #cell="{ record }">
              <a-tag :color="record.status===1?'green':record.status===0?'red':'yellow'">{{record.statusTypeMessage}}</a-tag>
            </template>
          </a-table-column>
          <a-table-column title="当前任务数" data-index="listNums"></a-table-column>
