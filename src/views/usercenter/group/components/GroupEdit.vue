...
import {Duplicate, getCanBeAdd, GroupUser, isDuplicate, updateGroup,} from '@/api/group';
...
props.groupData.groupUserVOList.forEach((item) => {
  tempvalue.push(item.userId);
});
const value = tempvalue;
selectUser.value = tempvalue;

const initData = async () => {
  const {data} = await getCanBeAdd(form.id);
  treeData.value = data;
...
.arco-transfer-view.arco-transfer-view-source {
  height: 400px;
}
</style>
