...
export default {
  ...
  methods: {
    ...
    async initFileList() {
      ...
      if (this.fieldModel) {
        ...
        if (Array.isArray(this.fieldModel)) {
          fileListM = deepClone(this.fieldModel);
        } else {
          fileListM.splice(0, 0, deepClone(this.fieldModel));
        }
        const promises = fileListM.map(async (file, index) => {
          const urls = file.url;
          if (urls) {
            const { data } = await getAllImageUrl({ key: urls });
            fileListM[index].url = data;
          }
        });

        await Promise.all(promises);
        this.fileList = deepClone(fileListM);
      }
    },
    ...
    resetField() {
      const { defaultValue } = this.field.options;
