...
export default {
  ...
  watch: {
    ...
    'field.options.checkField': {
      immediate: true,
      handler() {
        const nodeColumn = this.modeler.getNodeColumn;
        this.columnName = nodeColumn?.find(
          (t) => t.columnName === this.field.options.name
        )?.columnName;
      },
    },
    /**
    ...
    'field.options.name': {
      ...
      handler(value) {
        setTimeout(() => {
          ...
          const nodeColumn = modeler.getNodeColumn;
          this.columnName = nodeColumn?.find(
            (t) => t.columnName === value
          )?.columnName;
        }, 150);
      },
    },
