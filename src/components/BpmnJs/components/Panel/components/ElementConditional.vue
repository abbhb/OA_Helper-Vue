  ...
  <el-card shadow="never" class="container">
    ...
    <el-form label-width="80px">
      <template v-if="varVisible">
        ...
        <el-form-item v-if="varEventVisible" label="变量事件">
          <el-input
      ...
      >
        ...
        <el-table :data="list">
          ...
          <el-table-column label="字段" align="center">
            <template #default="scoped">
              <el-select v-model="scoped.row.field" size="small">
                <el-option-group
                  v-for="item in nodeWidgets"
                  :key="item.activityId"
                  :label="item.activityName"
                >
                  <el-option
                    v-for="widget in item.widgetList"
                    :key="widget.key"
          ...
          <el-table-column label="比较" width="80" align="center">
            <template #default="scoped">
              <el-select
                v-model="scoped.row.compare"
                placeholder="比较"
                size="small"
              >
                <el-option
                  v-for="compare in compareList"
                  :key="compare"
                  :label="compare"
                  :value="compare"
                />
          ...
          <el-table-column label="值" align="center">
            <template #default="scoped">
              <el-select
                v-if="getFieldType(scoped.row.field)?.type === 'Array'"
                v-model="scoped.row.value"
                size="small"
                placeholder="值"
              >
                <el-option
                  v-for="item in getFieldType(scoped.row.field)?.value"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                />
              ...
              <el-select
                v-else-if="getFieldType(scoped.row.field)?.type === 'Boolean'"
                v-model="scoped.row.value"
                size="small"
                placeholder="值"
              >
              ...
              <el-input
                v-else-if="getFieldType(scoped.row.field)?.type === 'Number'"
                v-model="scoped.row.value"
                size="small"
                placeholder="值"
                type="number"
        ...
        <div style="width: 100%">
          <el-button type="primary" plain style="width: 100%" @click="handleAdd"
          >新增
          </el-button
          >
        </div>
...
import {
  type NodeWidget,
  getWidgetTree,
} from '@/components/BpmnJs/bo-utils/variableUtil';
...
const getElementData = () => {
  ...
  if (conditionData.value.conditionType === 'expression') {
    conditionData.value.expression =
      CU.getConditionExpressionValue(scopedElement);
  }
};
...
const getFieldType = computed(
  () =>
    (field: string): WidgetType | undefined => {
      // eslint-disable-next-line no-use-before-define
      setExpression();
...
const getExpression = () => {
  ...
  const expression: string = conditionData.value.expression.replace(
    /^\${|\}$/g,
    ''
  );
 ...
 * 初始化
 */
eventHub.on('elementInit', function () {
