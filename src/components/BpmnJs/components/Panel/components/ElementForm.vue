<template>
  <el-card shadow="never">
    <el-divider
      v-if="scopedElement?.type === 'bpmn:StartEvent'"
      content-position="left"
    >主表单
    </el-divider
    >
    <el-divider
      v-if="scopedElement?.type === 'bpmn:UserTask'"
      content-position="left"
    >动态表单
    </el-divider
    >
    ...
    <div style="width: 100%">
      <el-button
        ...
        plain
        style="width: 100%"
        @click="selectFormRef.handleOpen()"
      >设计
      </el-button
      >
    </div>
    <SelectForm ref="selectFormRef" :form-json="formJson" @ok="selectFormOk"/>
  </el-card>
</template>
...
const getElementData = () => {
  formJson.value =
    modeler.getFormJsonList.find((t) => t.activityId === scopedElement.id)
      ?.formJson || {};
  ...
  if (widget.type === 'grid') {
    ...
    widget.rows.map((row) => {
      ...
      row.cols.map((cell) => {
        ...
        const rowChildren: any = rowNode.children;
        const cellNode: any = {
          id: cell.id,
          type: cell.type,
          icon: cell.icon,
...
eventBus.on('elementInit', function () {
  ...
  getElementData();
  if (
    Object.keys(formJson.value).length === 0 &&
    scopedElement.type === 'bpmn:UserTask'
  ) {
