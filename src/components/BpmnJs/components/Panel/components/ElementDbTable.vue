<template>
  <el-card shadow="never" class="container">
    ...
    <!-- 绑定状态 -->
    <el-descriptions v-if="detail.type !== ''" :column="1" border>
          ...
          <el-table :data="item.columns" style="width: 100%">
            ...
            <el-table-column prop="widgetIcon" label="控件类型">
              <template #default="scope">
                <svg-icon
                  v-if="scope.row.widgetIcon"
                  :icon-class="scope.row.widgetIcon"
                  class-name="color-svg-icon"
                />
                {{
                i18n.methods.i18n2t(
                `designer.widgetLabel.${scope.row.widgetType}`,
                `extension.widgetLabel.${scope.row.widgetType}`
                )
                }}
              </template>
            </el-table-column>
...
import {TableInfo} from '@/store/modules/modeler';
import {tableColumns, tableList} from "@/api/bpmn";
...
// 绑定的字段列表
const columnList = ref<NodeColumnInfo[]>([]);
...
const rules = ref({
  tableName: [
    ...
    },
    {
      ...
      validator: (rule, value) => {
        return (
          value.indexOf('sys_') === -1 &&
          value.indexOf('sql_') === -1 &&
          value.indexOf('act_') === -1
        );
      },
    },
    {
...
const getNodeColumns = () => {
  ...
  modeler.getNodeColumns.forEach((nodeItem) => {
    ...
    const formJson = modeler.getFormJsonList.find(
      (t) => t.activityId === nodeItem.activityId
    );
    nodeItem.columns.forEach((columnItem) => {
      const widgetItem = formJson?.formJson.widgetList.find(
        (t) => t.options.name === columnItem.columnName
      );
      columns.push({
    ...
    nodeColumnInfos.push({
      activityName:
      _elements[nodeItem.activityId]?.element.businessObject?.name,
      columns: columns,
    });
  });
...
const submitForm = () => {
  formRef.value.validate((valid: boolean) => {
    ...
    ElMessageBox.confirm(
      '确认要做更新操作吗?更新将删除所有已配置的字段',
      '提示'
    ).then(async () => {
      form.value.columns = [];
      if (form.value.type === 'ready') {

        const res = await tableColumns(form.value.tableName);
        form.value.columns = res.data;
      }
      // 只有选项是创建表的时候,更新数据才不会置空节点绑定的字段
      if (
        !(
          modeler.getTableInfo?.type === 'create' &&
