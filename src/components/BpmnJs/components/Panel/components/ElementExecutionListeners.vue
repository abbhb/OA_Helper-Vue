    ...
    <el-table :data="list">
      ...
      <el-table-column prop="" label="名称">
        <template #default="scoped">
          <span v-if="scoped.row.$attrs.name">{{
            scoped.row.$attrs.name
          }}</span>
      ...
      <el-table-column label="操作" width="120">
        <template #default="scope">
          <el-button link type="primary" @click="openListener(scope.$index)"
            >修改
          </el-button>
          <el-button link type="primary" @click="removeListener(scope.$index)"
            >删除
          </el-button>
        </template>
      </el-table-column>
            ...
            <el-form-item label="java类" prop="class" class="disabled-color">
              <el-input v-model="Lform.name" readonly="readonly">
                <template #append>
                  <el-button
                    :icon="Search"
                    @click="selectListenerRef.handleOpen()"
                  />
                </template>
              </el-input>
            </el-form-item>
...
<script lang="ts" setup>
  ...
  } from '@/components/BpmnJs/bo-utils/executionListenersUtil';
  ...
  const openListener = (index: number) => {
    ...
    nextTick(() => {
      ...
      if (index !== -1) {
        ...
        Lform.value.name = row.$attrs.name
          ? row.$attrs.name
          : row.$attrs['activiti:name'];
  ...
  const saveListener = () => {
    lformRef.value.validate((valid: boolean) => {
      if (!valid) return;
      lrowIndex === -1
        ? addExecutionListener(scopedElement, Lform.value)
        : updateExecutionListener(
            scopedElement,
            Lform.value,
            list.value[lrowIndex]
          );
      getElementData();
      LOpen.value = false;
    });
  ...
  eventBus.on('elementInit', function () {
    ...
    console.log('执行初始化？');
    let zhao = false;
    let zhaoElement = null;
    const zxq = 'com.qc.printers.common.activiti.listener.ApprovalTaskListener'
    for (let i = 0; i < list.value.length; i += 1) {
      if (list.value[i].class === zxq) {
        zhao = true;
        zhaoElement = list.value[i];
      }
    }
    if (scopedElement.type === 'bpmn:UserTask') {
      const form = {
        name: '审批监听',
        event: 'end',
        type: 'class',
      ...
      if (zhao) {
        updateExecutionListener(scopedElement, form, zhaoElement);
      } else {
        addExecutionListener(scopedElement, form);
      }
